...
function formatTime(minutes, seconds) {
  ...
  if (formattedMinutes.length === 1 || formattedSeconds.length === 1) {
...
function countVowels(str) {
  ...
  let sum = 0;
  for (let i = str.length - 1; i >= 0; i -= 1) {
    for (let k = vowels.length - 1; k >= 0; k -= 1) {
      if (str[i] === vowels[k]) {
        sum += 1;
      }
    }
...
function isPalindrome(str) {
  ...
  const palindrom = clearStr.split('').reverse().join('');
  // console.log(clearStr);
  // console.log(palindrom);
  if (clearStr === palindrom) {
    return true;
  }
  return false;
}
